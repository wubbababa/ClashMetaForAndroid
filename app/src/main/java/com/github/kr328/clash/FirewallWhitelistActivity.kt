...
class FirewallWhitelistActivity : BaseActivity<AccessControlDesign>() {
    override suspend fun main() {
        val store = ServiceStore(this)

        val selected = withContext(Dispatchers.IO) {
            store.firewallWhitelistPackages.toMutableSet()
        }

        defer {
            withContext(Dispatchers.IO) {
                val changed = selected != store.firewallWhitelistPackages
                store.firewallWhitelistPackages = selected

                // Whitelist changes take effect by hot-reloading the running configuration.
                if (changed && clashRunning && store.firewallEnabled) {
                    val active = store.activeProfile

                    if (active != null)
                        sendProfileChanged(active)
                }
            }
        }
